...
BEGIN
    ...
    safe_drop('DROP TRIGGER TRG_PK_CAT_TIPO_SERV');
    ...
    safe_drop('DROP TABLE JRGY_SERVICIO_PRODUCTO CASCADE CONSTRAINTS');
    safe_drop('DROP TABLE JRGY_SERVICIO_PAQUETE CASCADE CONSTRAINTS');
    ...
    safe_drop('DROP TABLE JRGY_CAT_TIPO_SERVICIO CASCADE CONSTRAINTS');
    ...
    safe_drop('DROP SEQUENCE SQ_PK_CAT_TIPO_SERV');
...
-- ========================================
-- Procedimientos: Paquete -> Servicios incluidos
-- ========================================
CREATE OR REPLACE PROCEDURE JRGY_PRO_PAQ_SERV_LISTAR(
...
CREATE OR REPLACE PROCEDURE JRGY_PRO_PAQ_SERV_REEMPLAZAR(
    P_PAQUETE_ID IN NUMBER,
    P_ITEMS_JSON IN CLOB
)
IS
...
-- ========================================
-- Catálogo: habilidades de empleado por tipo (servicio/producto)
-- ========================================
BEGIN
    EXECUTE IMMEDIATE 'DROP TABLE JRGY_EMPLEADO_HABILIDAD';
EXCEPTION
    WHEN OTHERS THEN NULL;
END;
/

CREATE TABLE JRGY_EMPLEADO_HABILIDAD (
    COD_EMPLEADO NUMBER NOT NULL,
    CATEGORIA VARCHAR2(50) NOT NULL,
    TIPO VARCHAR2(20) NOT NULL, -- SERVICIO / PRODUCTO
    CONSTRAINT PK_JRGY_EMP_HAB PRIMARY KEY (COD_EMPLEADO, CATEGORIA, TIPO),
    CONSTRAINT FK_JRGY_EMP_HAB_EMP FOREIGN KEY (COD_EMPLEADO) REFERENCES JRGY_EMPLEADO (COD_EMPLEADO)
);

CREATE OR REPLACE PROCEDURE JRGY_PRO_EMP_HAB_LISTAR(
    P_EMP_ID IN NUMBER,
    CUR OUT SYS_REFCURSOR
)
IS
...
CREATE OR REPLACE PROCEDURE JRGY_PRO_EMP_HAB_REEMPLAZAR(
    P_EMP_ID IN NUMBER,
    P_ITEMS_JSON IN CLOB
)
IS
...
CREATE TABLE JRGY_CAT_TIPO_SERVICIO (
    COD_TIPO_SERVICIO NUMBER,
    NOMBRE VARCHAR2(50) NOT NULL,
    DESCRIPCION VARCHAR2(200),
    CONSTRAINT PK_JRGY_CAT_TIPO_SERVICIO PRIMARY KEY (COD_TIPO_SERVICIO),
    CONSTRAINT UQ_JRGY_CAT_TIPO_SERVICIO UNIQUE (NOMBRE)
);
...
CREATE TABLE JRGY_PRODUCTO (
    ...
    CONSTRAINT FK_JRGY_PRODUCTO_TIPO FOREIGN KEY (COD_TIPO_SERVICIO) REFERENCES JRGY_CAT_TIPO_SERVICIO (COD_TIPO_SERVICIO)
);
...
CREATE TABLE JRGY_SERVICIO (
    ...
    CONSTRAINT FK_JRGY_SERVICIO_TIPO FOREIGN KEY (COD_TIPO_SERVICIO) REFERENCES JRGY_CAT_TIPO_SERVICIO (COD_TIPO_SERVICIO),
...
CREATE TABLE JRGY_SERVICIO_PAQUETE (
    COD_PAQUETE NUMBER NOT NULL,
    COD_SERVICIO_INCL NUMBER NOT NULL,
    CONSTRAINT PK_JRGY_SERVICIO_PAQUETE PRIMARY KEY (COD_PAQUETE, COD_SERVICIO_INCL),
    CONSTRAINT FK_JRGY_SERV_PAQ_PQ FOREIGN KEY (COD_PAQUETE) REFERENCES JRGY_SERVICIO (COD_SERVICIO) ON DELETE CASCADE,
    CONSTRAINT FK_JRGY_SERV_PAQ_SERV FOREIGN KEY (COD_SERVICIO_INCL) REFERENCES JRGY_SERVICIO (COD_SERVICIO) ON DELETE CASCADE
);
/
...
CREATE SEQUENCE SQ_PK_CAT_TIPO_SERV START WITH 1 INCREMENT BY 1 NOMAXVALUE NOCYCLE;
CREATE OR REPLACE TRIGGER TRG_PK_CAT_TIPO_SERV BEFORE INSERT ON JRGY_CAT_TIPO_SERVICIO FOR EACH ROW
...
DECLARE
    ...
    PROCEDURE ensure_cat_tipo_serv(p_codigo NUMBER, p_nombre VARCHAR2, p_desc VARCHAR2) IS
    BEGIN
        MERGE INTO JRGY_CAT_TIPO_SERVICIO t
        USING (SELECT p_codigo AS COD, p_nombre AS NOM, p_desc AS DESCR FROM dual) s
        ON (UPPER(t.NOMBRE) = UPPER(s.NOM))
        WHEN NOT MATCHED THEN
            INSERT (COD_TIPO_SERVICIO, NOMBRE, DESCRIPCION) VALUES (s.COD, s.NOM, s.DESCR);
    END;
...
BEGIN
    ...
    ensure_cat_tipo_serv(1, 'SPA', 'Servicios de bienestar y spa');
    ensure_cat_tipo_serv(2, 'LIMPIEZA', 'Servicios de limpieza y housekeeping');
    ensure_cat_tipo_serv(3, 'TOUR', 'Tours y actividades externas');
    ensure_cat_tipo_serv(4, 'RESTAURANTE', 'Consumos de restaurante/bar');
    ensure_cat_tipo_serv(5, 'OTROS', 'Servicios generales');
